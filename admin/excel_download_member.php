...
$media_query	= "SELECT mb_media, COUNT( mb_media ) media_cnt FROM ".$_gl['member_info_table']." WHERE 1 AND mb_regdate LIKE '%".$todayDate."%' GROUP BY mb_media";
...
while ($media_daily_data = mysqli_fetch_array($media_res))
{
    ...
    $pc_query		= "SELECT * FROM ".$_gl['member_info_table']." WHERE 1 AND mb_regdate LIKE  '%".$todayDate."%' AND mb_media='".$media_daily_data['mb_media']."' AND mb_gubun='PC' ";
    $pc_count		= mysqli_num_rows(mysqli_query($my_db, $pc_query));
    $mobile_query	= "SELECT * FROM ".$_gl['member_info_table']." WHERE 1 AND mb_regdate LIKE  '%".$todayDate."%' AND mb_media='".$media_daily_data['mb_media']."' AND mb_gubun='MOBILE'";
...
foreach($media_name as $key => $val)
{
    ...
    $objPHPExcel->setActiveSheetIndex(0)
                ...
                ->setCellValue('E'.$dataIdx, number_format($media_cnt[$i]));
...
// Rename worksheet
$objPHPExcel->getActiveSheet()->setTitle('현대해상어서와마음봇_참여자데이터_'.$todayDate);
...
header('Content-Disposition: attachment;filename="현대해상어서와마음봇_참여자데이터_'.$todayDate.'.xls"');
